...
function jd_core_decode_manifest_response($body){
    ...
    // GitHub Contents API may ignore the raw Accept header and return base64 JSON.
...
function jd_core_remote_manifest($force=false){
    ...
    foreach ($endpoints as [$url, $headers]) {
        ...
        if (is_wp_error($r)) {
            ...
            jd_core_store_update_diag(false, $url, 0, $last_error);
            continue;
        }
        ...
        $last_error = 'HTTP ' . $status . ' ou manifeste invalide';
        jd_core_store_update_diag(false, $url, $status, $last_error);
    }
    return null;
}

/** Native WordPress Update URI channel (WP 5.8+). */
...
/** Fallback injection for hosts/plugins that alter core update transients. */
...
/** Keep the plugin directory stable even if a package extracts to a different folder. */
...
/** Manual diagnostic / force check used from the John Design admin screen. */
